...
class Application(Frame):
    ...
    def dirStart(self):
        ...
        if self.cbAlgorithm.current() == 0 and len(key) != 16 and len(key) != 24 and len(key) != 32:
		    print 'sai key 0'
		    tkMessageBox.showerror('Error', 'The key for AES algorithm is 16 or 24 or 32 bytes long')
	    elif self.cbAlgorithm.current() == 2 and len(key) != 16 and len(key) != 24 and len(key) != 32:
		    print 'sai key 2'   
		    tkMessageBox.showerror('Error', 'The key for DES3 algorithm is 16 or 24 or 32 bytes long')  
